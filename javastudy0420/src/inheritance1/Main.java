package inheritance1;

public class Main {

	public static void main(String[] args) {
		//Employee 인스턴스 생성
		//어떤 클래스의 인스턴스를 1개만 만드는 경우
		//인스턴스의 참조를 저장할 변수를 클래스의 이름과
		//동일하게 하고 첫글자만 소문자로 하는 경우가 많음
		Employee employee = new Employee();
		employee.setEmpName("배기훈");
		employee.setEmpPart("프로그래머");
		System.out.println(employee.resultStr());
		
		//Manager 클래스의 인스턴스를 생성하고 값을 대입		
		Manager manager = new Manager();
		manager.setEmpName("배성민");
		manager.setEmpPart("개발자");
		manager.setRank("팀장");
		System.out.println(manager.managerStr());

		//Employee 인스턴스를 1개 더 생성하고 값을 대입
		//첫번째와 다른 생성자를 이용
		Employee employee1 = new Employee("임성현","앱개발자");
		System.out.println(employee1.resultStr());
		System.out.println();
		

	}

}
